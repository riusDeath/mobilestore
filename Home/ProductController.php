...
class ProductController extends Controller
{
    ...
    public function product_brand($brand_id)
    {
        ...
        $mess = $brand->name;
        $products = Product::where('status', '1')->productBrand($brand_id)
        ->paginate(12)->appends('brand_id', $brand_id);
    ...
    public function product_price(Request $request)
    {
        ...
        $products = Product::productPrice()->where('status', '1')
        ->paginate(12)->appends(['value_min' => $request->value_min, 'value_max'=> $request->value_max]);
    ...
    public function product_start(Request $request)
    {
        if (!empty($request->rate)) {
            ...
            ->paginate(12)->appends('rate', $request->rate);
    ...
    public function addComment(Request $request, $id)
    {
        ...
        ->with('thongbao', 'Thêm bình luận thành công');
    }
